class Admin::EventsController < Admin::BaseController
  ...
  def auto_emails
    ...
    if inscription_responsable.blank?
      ...
      return redirect_to admin_events_path(month: l(@diner.start_date, format: '%B %Y').humanize)
    end
    ...
    recipients = Event.where(start_date: @date).where.not(event_type: "Petit-dejeuner")
                      .joins(:inscriptions)
                      .joins(:volunteers)
                      .pluck('volunteers.email') +
                 Event.where(start_date: @date + 1).where(event_type: "Petit-dejeuner")
                      .joins(:inscriptions)
                      .joins(:volunteers)
                      .pluck('volunteers.email')
    @email_recipients = recipients.reject { |c| c.empty? }.uniq

    @email2_subject = "Soirée Hiver Solidaire du #{l(@date, format: :long)} (Note au responsable)"
    @email2_recipient = @responsable.email unless @diner.mail_sent

    if request.method == 'POST'
      UserMailer.recapitulatif_soiree(params[:email_subject], params[:email_content], @email_recipients).deliver_later
      UserMailer.recapitulatif_soiree(params[:email2_subject], params[:email2_content], @email2_recipient).deliver_later
      flash[:notice] = "Emails envoyés !"
      @diner.update(mail_sent: true)
      redirect_to admin_events_path(month: l(@diner.start_date, format: '%B %Y').humanize)
    end
  end
